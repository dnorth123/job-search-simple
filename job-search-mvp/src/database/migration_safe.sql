...
-- Run this in your Supabase SQL Editor (no destructive operations)
...
-- Create trigger for automatic timeline creation (only if it doesn't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'create_initial_timeline_entry') THEN
